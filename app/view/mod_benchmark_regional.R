...
server <- function(id, wbes_data, global_filters = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    output$performance_growth_distribution <- renderPlotly({
      ...
      plot_ly(data, x = ~reorder(region, annual_sales_growth_pct), y = ~annual_sales_growth_pct,
        ...
        layout(title = list(text = "Sales Growth by Region", font = list(size = 14)),
               xaxis = list(title = "", tickangle = -45, automargin = TRUE),
               yaxis = list(title = "Annual Sales Growth (%)", automargin = TRUE),
               margin = list(l = 60, r = 120, t = 40, b = 120),
    ...
    output$dl_infra_map <- downloadHandler(
      filename = function() { paste0("infrastructure_map_regional_", Sys.Date(), ".html") },
      content = function(file) {
        map <- create_wbes_map_3d(
          data = comparison_data(),
          coordinates = get_country_coordinates(wbes_data()),
          indicator_col = input$infra_indicator,
          group_col = input$region_grouping,
          title = "Infrastructure Indicators by Region"
        )
    ...
    output$dl_finance_map <- downloadHandler(
      filename = function() { paste0("finance_map_regional_", Sys.Date(), ".html") },
      content = function(file) {
        map <- create_wbes_map_3d(
          data = comparison_data(),
          coordinates = get_country_coordinates(wbes_data()),
          indicator_col = input$finance_indicator,
          group_col = input$region_grouping,
          title = "Finance Indicators by Region"
        )
        htmlwidgets::saveWidget(map, file)
      }
    )
    ...
    output$dl_governance_map <- downloadHandler(
      filename = function() { paste0("governance_map_regional_", Sys.Date(), ".html") },
      content = function(file) {
        map <- create_wbes_map_3d(
          data = comparison_data(),
          coordinates = get_country_coordinates(wbes_data()),
          indicator_col = input$governance_indicator,
          group_col = input$region_grouping,
          title = "Governance Indicators by Region"
        )
        htmlwidgets::saveWidget(map, file)
      }
    )
    ...
    output$dl_workforce_map <- downloadHandler(
      filename = function() { paste0("workforce_map_regional_", Sys.Date(), ".html") },
      content = function(file) {
        map <- create_wbes_map_3d(
          data = comparison_data(),
          coordinates = get_country_coordinates(wbes_data()),
          indicator_col = input$workforce_indicator,
          group_col = input$region_grouping,
          title = "Workforce Indicators by Region"
        )
        htmlwidgets::saveWidget(map, file)
      }
    )
    ...
    output$dl_performance_map <- downloadHandler(
      filename = function() { paste0("performance_map_regional_", Sys.Date(), ".html") },
      content = function(file) {
        map <- create_wbes_map_3d(
          data = comparison_data(),
          coordinates = get_country_coordinates(wbes_data()),
          indicator_col = input$performance_indicator,
          group_col = input$region_grouping,
          title = "Performance Indicators by Region"
        )
        htmlwidgets::saveWidget(map, file)
      }
    )
    ...
    output$dl_crime_map <- downloadHandler(
      filename = function() { paste0("crime_map_regional_", Sys.Date(), ".html") },
      content = function(file) {
        map <- create_wbes_map_3d(
          data = comparison_data(),
          coordinates = get_country_coordinates(wbes_data()),
          indicator_col = input$crime_indicator,
          group_col = input$region_grouping,
          title = "Crime Indicators by Region"
        )
        htmlwidgets::saveWidget(map, file)
      }
    )
