...
server <- function(id, wbes_data, global_filters = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    create_domain_chart <- function(data, domain_key, chart_type = "bar") {
      ...
      if (chart_type == "bar") {
        if (has_grouping) {
          ...
          # No grouping - simple bar chart with different colors per sector, indicator name as x-axis title
          sectors <- unique(data$sector)
          colors <- c("#1B6B5F", "#F49B7A", "#2E7D32", "#17a2b8", "#6C757D", "#F4A460", "#dc3545", "#9c27b0")

          # Create stable color mapping for sectors
          color_map <- setNames(colors[((seq_along(sectors) - 1) %% length(colors)) + 1], sectors)
          ...
          plots <- lapply(seq_along(available), function(i) {
            ...
            # Create bar chart with different color per sector
    ...
    # ============================================================
    # Statistical Tests for Cross-Sector Comparisons
    # ============================================================

    # Helper function to create statistical tests for sector comparisons
    create_sector_stats <- function(data, indicator, indicator_label) {
      ...
      # Perform test
      result <- anova_with_tukey(df, "value", "sector")
      format_anova_results(result, title = paste(indicator_label, "- Sector Comparison"))
    }
    ...
    output$overview_stats <- renderUI({
      ...
      if (nrow(d) < 3) {
        return(tags$div(class = "text-muted small", "Select at least 3 sectors for statistical analysis"))
      }

      # Run ANOVA for key indicators across all domains
      indicators <- c(
      ...
      results <- lapply(names(indicators), function(ind) {
        create_sector_stats(d, ind, indicators[[ind]])
      })
