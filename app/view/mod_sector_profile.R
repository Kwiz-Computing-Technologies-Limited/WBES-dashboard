...
server <- function(id, wbes_data, global_filters = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    # Geographic map for sector profile
    output$sector_profile_map <- renderLeaflet({
      req(sector_data(), wbes_data(), input$map_indicator)
      d <- sector_data()
    ...
    # MIRRORS Infrastructure domain module - aggregated by sector
    ...
    output$infra_chart1 <- renderPlotly({
      ...
      # Build metrics using SAME columns as Infrastructure domain (aggregated by sector)
      metrics <- data.frame(
        category = character(),
        value = numeric(),
      ...
      if (nrow(metrics) > 0) {
        plot_ly(metrics,
          ...
          layout(
            title = list(text = "Infrastructure Metrics (Sector Avg)", font = list(size = 14)),
            xaxis = list(title = ""),
        ...
        plot_ly() |>
          layout(
            annotations = list(
              text = paste0("No infrastructure data available for this sector<br>",
    ...
    output$infra_chart2 <- renderPlotly({
      ...
      # MIRRORS Infrastructure domain - Power Source Distribution (aggregated by sector)
      ...
      if (!is.na(generator_pct)) {
        ...
        plot_ly(
          ...
          layout(
            title = list(text = "Power Source Distribution (Sector)", font = list(size = 14)),
            showlegend = FALSE,
            paper_bgcolor = "rgba(0,0,0,0)"
          ) |>
          config(displayModeBar = FALSE)
    ...
    # Governance Charts
    # MIRRORS Corruption domain module - aggregated by sector
    # Uses IC.FRM.CORR.ZS and IC.FRM.BRIB.ZS (same as mod_corruption.R)
    output$gov_chart1 <- renderPlotly({
      req(sector_data())
      d <- sector_data()

      # Build governance metrics using SAME columns as Corruption domain (aggregated by sector)
      metrics <- data.frame(
        indicator = character(),
        value = numeric(),
        stringsAsFactors = FALSE
      ...
      if (nrow(metrics) > 0) {
        plot_ly(metrics,
          ...
          layout(
            title = list(text = "Governance & Corruption Metrics (Sector Avg)", font = list(size = 14)),
            xaxis = list(title = ""),
            yaxis = list(title = "% of Firms", range = c(0, 100)),
            paper_bgcolor = "rgba(0,0,0,0)",
            plot_bgcolor = "rgba(0,0,0,0)"
        ...
        plot_ly() |>
          layout(
            annotations = list(
              text = paste0("No governance data available for this sector<br>",
